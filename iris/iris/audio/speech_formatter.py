...
_PHRASE_REPLACEMENTS: Final[tuple[tuple[str, str], ...]] = (
    (r"요청하신 작업을 수행하겠습니다\.?", "바로 진행할게요."),
    (r"네,\s*요청하신 작업을 수행하겠습니다\.?", "좋아요. 바로 실행할게요."),
    (r"작업을\s*수행하겠습니다\.?", "진행할게요."),
    (r"현재\s*등록된\s*모니터링\s*대상이\s*없습니다\.?", "아직 감시 중인 작업은 없어요. 시작하시면 같이 볼게요."),
...
def _tone_adjust(text: str, tone: AppState) -> str:
    """상태별 말투·짧은 반응."""
    base = text.strip()
    if not base:
        return "네."
    ...
    if tone is AppState.PROCESSING:
        if any(k in base for k in ("잠깐", "확인", "처리")):
            return _limit_sentences(base, max_sentences=2, max_chars=200)
        return _limit_sentences("잠깐만요. " + base, max_sentences=3, max_chars=320)

    if tone is AppState.EXECUTING:
        if not any(k in base for k in ("실행", "진행", "할게", "시작")):
            return _limit_sentences("좋아요. " + base, max_sentences=3, max_chars=320)
        return _limit_sentences(base, max_sentences=3, max_chars=320)

    if tone is AppState.ALERTING:
        if not any(k in base for k in ("멈춘", "문제", "확인", "같")):
            return _limit_sentences("확인해보니, " + base, max_sentences=3, max_chars=320)
        return _limit_sentences(base, max_sentences=3, max_chars=320)
...
def format_speech(text: str, tone: AppState) -> str:
    """
    채팅에 표시된 원문을 음성용 짧은 구어체로 변환.

    - 최대 2~3문장
    - 문장 사이 짧은 쉼: 줄바꿈(엣지/엔진이 자연스러운 호흡으로 읽기 쉬움)
    """
    raw = (text or "").strip()
    ...
    toned = _tone_adjust(softened, tone)
