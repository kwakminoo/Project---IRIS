...
class ComputerUseAgent:
    ...
    def run(self, goal: str, slots: dict[str, Any] | None = None) -> str:
        ...
        # 미디어 스킬 — Router skill_id + slots (CU 플래너 우회)
        from iris.assistant.media_playback_flow import MediaPlaybackFlow, should_run_media_flow

        skill_id = str(slot_map.get("skill_id") or "").strip()
        if skill_id == "media_play" or should_run_media_flow(slot_map):
            push_activity_line("ComputerUse: skill=media_play MediaPlaybackFlow.")
